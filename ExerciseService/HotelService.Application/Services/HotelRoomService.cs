using HotelService.Application.Interfaces.Halpers;
    ...
    : IHotelRoomService
{
    ...
    public async Task<bool> DeleteHotelRoomAsync(Guid roomId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            await roomStateService.DeleteRoomStatesByHotelRoomIdAsync(roomId, ct);
            
            await hotelRoomRepository.DeleteByHotelIdAsync(roomId, ct);
    ...
    {
        await roomStateService.DeleteRoomStatesByHotelIdAsync(hotelId, ct);
        await roomPhotoService.DeleteRoomPhotosByHotelIdAsync(hotelId, ct);
        
        await hotelRoomRepository.DeleteByHotelIdAsync(hotelId, ct);
